...
save_checkpoint:
	...
	ldw t0, SCORE(zero)                 ; takes the value of the score
 	add t1, zero, zero                  ; prepares the quotient q (now =0)
    bne t0, zero, moduloopSave 
	jmpi endSave

moduloopSave:
	addi t2, t0, -10				  ; compute the number of time we have 10 in t0 by substracting 10
	blt  t2, zero, endModuloopSave		  ; if value is smaller than zero, result : t0=the result of the modulo 10
	addi t1, t1, 1                    ; increments quotient value
	add  t0, t2, zero                 ; computes new rest value
	jmpi moduloopSave
...
endModuloopSave:
	...
	addi t5, zero, GSA			; computing of limit value for a0
	addi t5, t5, 384			; computing of limit value for a0=> GSAaddr + 96*4
